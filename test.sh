...
grep -q '^SCRIPT_VERSION="0.0.6"' "${SCRIPT_DIR}/install.sh" && ok "SCRIPT_VERSION 0.0.6" || fail "SCRIPT_VERSION"
bash "${SCRIPT_DIR}/install.sh" --version 2>/dev/null | grep -q 'v0.0.6' && ok "install.sh --version" || fail "install.sh --version"
