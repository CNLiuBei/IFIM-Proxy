...
sha=$(curl -fsSL --max-time 15 "https://api.github.com/repos/${GITHUB_REPO}/commits/main" \
    | sed -n 's/.*"sha": "\([a-f0-9]\{40\}\)".*/\1/p' | head -1)

if [[ -n "${sha}" ]]; then
    curl -fsSL "https://raw.githubusercontent.com/${GITHUB_REPO}/${sha}/assets/subscribe-panel.html" \
        -o "${PANEL_DIR}/index.html"
else
    curl -fsSL "https://raw.githubusercontent.com/${GITHUB_REPO}/main/assets/subscribe-panel.html?v=$(date +%s)" \
        -o "${PANEL_DIR}/index.html"
fi

grep -q 'cards-row' "${PANEL_DIR}/index.html" \
    || { echo "错误: 下载的仍是旧版页面，请稍后重试"; exit 1; }
...
if "__PANEL_CONFIG__" not in html:
    raise SystemExit("错误: 页面模板异常，缺少 __PANEL_CONFIG__")
...
echo "订阅页已刷新（三列布局）: ${panel_url}"
echo "请用浏览器 Ctrl+F5 或无痕窗口打开上述链接"
